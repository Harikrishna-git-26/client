...
export default function App() {
    ...
    socket.on("offer", ({ from, signal, name: peerName }) => {
      ...
      if (mediaStream) {
        mediaStream.getTracks().forEach(track => {
          try { peer.addTrack(track, mediaStream); } catch {}
        });
      }
  ...
  const connectToFriend = () => {
    if (!targetId.trim() || targetId.length !== 5) return alert("Enter valid 5-character ID");
  ...
  const broadcastAction = (action) => {
    ...
    peers.forEach(p => socketRef.current?.emit("send-video", { to: p.id, action, time }));
  };
            ...
            {peers.filter(p => p.id !== socketId).map(p => (
              <div key={p.id} style={{ background: "#292f42", borderRadius: 8, minHeight: 120, display: "flex", alignItems: "center", gap: 20, padding: "0 16px" }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  ...
                  {hostId === socketId && <button onClick={() => removePeer(p.id)} style={{ color: "#dc2626", background: "#fff", border: "1px solid #dc2626", borderRadius: 8, padding: "4px 12px", cursor: "pointer" }}>Remove</button>}
                </div>
              </div>
            ))}
          </div>
