...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Chat messages - avoid duplicating own message
  ...
  // WebCam & Mic start/stop & WebRTC peer init
  const startCamMic = () => {
  ...
  // Connect to friend with validation
  const connectToFriend = () => {
    if (!targetId.trim() || targetId.length !== 5) {
      alert("Please enter valid 5-character Socket ID");
      return;
    }
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif", display: "flex", flexDirection: "column", height: "100vh", width: "100vw", boxSizing: "border-box", background: "#090d14", color: "#eee", overflow: "hidden" }}>
      {/* Header */}
      <div style={{ display: "flex", flexShrink: 0, justifyContent: "space-between", alignItems: "center", padding: "10px 20px", background: "#121826", borderBottom: "1px solid #1e2536" }}>
        <div style={{ fontWeight: 700, fontSize: 28 }}>WatchApp</div>
        {nameSet ? (
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <span style={{ fontSize: 20 }}>{name}</span>
            <span style={{ background: "#2d364d", padding: "4px 12px", borderRadius: 12, fontWeight: 700, letterSpacing: 2, fontSize: 18 }}>
              {socketId}
            </span>
          </div>
        ) : (
          <input autoFocus value={name} placeholder="Enter your name" onChange={e => setName(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && name.trim()) setNameSet(true); }} style={{ padding: "8px 18px", borderRadius: 10, border: "1px solid #555", background: "#1a1f2c", color: "#eee", fontSize: 20 }} />
        )}
      </div>
